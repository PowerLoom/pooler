            ...
            for i, j in logs:
                ...
                if i['update']['action'].lower().find('publish') != -1:
                    ...
                    typer.echo(
                        ...
                        f"{i['update']['action']} - {contracts_len} contracts\n",
                    )
...
def dagChainStatus(dag_chain_height: int = typer.Argument(-1)):
    ...
    if total_issue_count['LAG_EXIST_IN_DAG_CHAIN'] > 3:
        total_issue_count[
            'LAG_EXIST_IN_DAG_CHAIN'
        ] = f"THERE IS A LAG WHILE PROCESSING CHAIN, BIGGEST LAG: {total_issue_count['LAG_EXIST_IN_DAG_CHAIN']}"
    else:

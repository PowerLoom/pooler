...
async def get_pair_reserves(
    ...
    try:
        ...
        if not rate_limit_lua_script_shas:
            rate_limit_lua_script_shas = await load_rate_limiter_scripts(
                redis_conn,
            )
        ...
        await check_rpc_rate_limit(
            ...
            error_msg={
                'msg': 'exhausted_api_key_rate_limit inside get_pair_reserves',
            },
            logger=core_logger,
            rate_limit_lua_script_shas=rate_limit_lua_script_shas,
        ...
        if not reserves_array:
            core_logger.trace(
                (
                    'Unable to fetch reserves for pair: {}, from block: {}, to'
                    ' block: {} got {} from RPC'
                ),
                pair_address,
                from_block,
            ...
            exc = RPCException(
                ...
                response=reserves_array,
                underlying_exception=None,
                extra_info={
                    'msg': 'Error: failed to retrieve pair reserves from RPC',
                },
            )
            raise exc
        ...
        raise RPCException(
            ...
            underlying_exception=None,
            extra_info={
                'msg': f'Error: get_pair_reserves error_msg: {str(exc)}',
            },
        ) from exc
...
async def get_pair_trade_volume(
    ...
    try:
        ...
        await check_rpc_rate_limit(
            ...
            logger=core_logger,
            rate_limit_lua_script_shas=rate_limit_lua_script_shas,
            limit_incr_by=max_chain_height - min_chain_height + 1,
        )
        events_log = await ev_loop.run_in_executor(
            func=pfunc_get_event_logs, executor=None,
        )
        ...
        raise RPCException(
            ...
            underlying_exception=None,
            extra_info={
                'msg': f'error: get_pair_trade_volume, error_msg: {str(exc)}',
            },
        ) from exc

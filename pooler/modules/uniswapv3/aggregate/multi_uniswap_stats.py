...
class AggregateStatsProcessor(GenericProcessorMultiProjectAggregate):
    ...
    async def compute(
        ...
        # TODO add genesis functionality
        ...
        for snapshot_project_id in snapshot_mapping.keys():
            ...
            if "reserves" in snapshot_project_id:
                ...
                # BUG is this logic correct? seems like it will incorrectly sum tvl due to +=
                stats_data["tvl"] += (
        ...
        if not extrapolated_flag:
            ...
            if previous_stats_snapshot_data:
                ...
                stats_data["volumeChange24h"] = (
                    (stats_data["volume24h"] - previous_stats_snapshot.volume24h)
                    / previous_stats_snapshot.volume24h
                    * 100
                )

                stats_data["tvlChange24h"] = (
                    (stats_data["tvl"] - previous_stats_snapshot.tvl)
                    / previous_stats_snapshot.tvl
                    * 100
                )

                stats_data["feeChange24h"] = (
                    (stats_data["fee24h"] - previous_stats_snapshot.fee24h)
                    / previous_stats_snapshot.fee24h
                    * 100
                )

...
class AggreagateTradeVolumeProcessor(GenericProcessorSingleProjectAggregate):
    ...
    async def compute(
        ...
        aggregate_snapshot = UniswapTopPair7dSnapshot(
            name=pair_metadata['pair']['symbol'], address=contract, volume7d=0, fee7d=0,
        )
        ...
        count = 0
        self._logger.debug(
            'fetch # {}: queueing task for 24h aggregate snapshot for '
            'project ID {} at currently received epoch ID {} with snasphot CID {}',
        ...
        self._logger.debug(
            'for 7d aggregated trade volume calculations: fetched {} 24h aggregated'
            ' trade volume snapshots for project ID {}: {}', len(
                all_snapshots,

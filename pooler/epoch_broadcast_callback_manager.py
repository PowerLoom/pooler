...
class EpochCallbackManager(Process):
    ...
    def _epoch_broadcast_callback(self, dont_use_ch, method, properties, body):
        ...
        with create_redis_conn(self._connection_pool) as r:
            ...
            to_be_pruned_ts = (
                settings.epoch.height * settings.epoch.block_time * 20
            )
            ...
            if older_broadcast_ids:
                older_broadcast_ids_dec = map(
                    lambda x: x.decode('utf-8'), older_broadcast_ids,
                )
                [
                    r.delete(
    ...
    def run(self) -> None:
        ...
        self._logger.debug(
            'Launched PowerLoom|EpochCallbackManager with PID: {}', self.pid,
        )

...
router_contract_abi = read_json_file(worker_settings.uniswap_contract_abis.router)
pair_contract_abi = read_json_file(worker_settings.uniswap_contract_abis.pair_contract)
...
async def get_token_price_at_block_height(
    ...
    try:
        ...
        stable_coins_addresses = {
            'USDC': Web3.toChecksumAddress(worker_settings.contract_addresses.USDC),
            'DAI': Web3.toChecksumAddress(worker_settings.contract_addresses.DAI),
            'USDT': Web3.toChecksumAddress(worker_settings.contract_addresses.USDT),
        }
        ...
        non_stable_coins_addresses = {
            Web3.toChecksumAddress(worker_settings.contract_addresses.agEUR): {
                'token0': Web3.toChecksumAddress(worker_settings.contract_addresses.agEUR),
                'token1': Web3.toChecksumAddress(worker_settings.contract_addresses.FEI),
                'decimals': 18,
            },
            Web3.toChecksumAddress(worker_settings.contract_addresses.SYN): {
                'token0': Web3.toChecksumAddress(worker_settings.contract_addresses.SYN),
                'token1': Web3.toChecksumAddress(worker_settings.contract_addresses.FRAX),
                'decimals': 18,
            },
        }
        ...
        if Web3.toChecksumAddress(token_metadata['address']) in list(stable_coins_addresses.values()):
            ...
            if debug_log:
                print(
                    f"## {token_metadata['symbol']}: ignored stablecoin calculation for token0: {token_metadata['symbol']} - WETH - USDT conversion: {token_price}",
                )
        ...
        # check if token has no pair with stablecoin and weth if so then use hardcoded path
            ...
            if temp_token_price:
                # USDC decimals
                temp_token_price = temp_token_price[2] / \
                    10**stable_coins_decimals['USDC'] if temp_token_price[2] != 0 else 0
        ...
        # 3. if price is still 0/None then increase token amount in path (token->weth-usdc)
            ...
            for key, value in stable_coins_addresses.items():
                try:
                    ...
                    if temp_token_price:
                        # USDT decimals
                        temp_token_price = temp_token_price[1] / \
                            10**stable_coins_decimals[key] if temp_token_price[1] != 0 else 0

                        print(
                            f"## {token_metadata['symbol']}->{key}: token price: {temp_token_price}",
                        )
                    ...
                    if temp_token_price == 0:
                        ...
                        if temp_token_price:
                            ...
                            print(
                                f"## {token_metadata['symbol']}->{key}: (increased_input_amount) token price : {temp_token_price}",
                            )
            ...
            print(
                f"## {token_metadata['symbol']}: chosed token price after all stable coin conversions: {token_price}",
            )
            ...
            try:
                ...
                if temp_token_price:
                    # USDT decimals
                    temp_token_price = temp_token_price[2] / \
                        10**stable_coins_decimals['USDT'] if temp_token_price[2] != 0 else 0
                    print(
                        f"## {token_metadata['symbol']}: token price after weth->stablecoin: {temp_token_price}",
                    )
                    token_price = temp_token_price if token_price < temp_token_price else token_price
            except Exception:
            ...
            if token_price == 0:
                ...
                if temp_token_price:
                    ...
                    temp_token_price = temp_token_price / token_amount_multiplier
                    print(
                        f"## {token_metadata['symbol']}: token price after weth->stablecoin (increased_input_amount): {temp_token_price}",
                    )
                    token_price = temp_token_price if token_price < temp_token_price else token_price
        ...
        print(
            f"Error: failed to fetch token price | error_msg: {str(err)} | contract: {token_metadata['address']}",
        )
    finally:
    ...
    for contract in all_contracts:
        ...
        token0, token1 = await asyncio.gather(
            get_token_price_at_block_height(
                router_contract_obj, pair_per_token_metadata['token0'], 'latest', loop, redis_conn,
            ),
            get_token_price_at_block_height(
                router_contract_obj, pair_per_token_metadata['token1'], 'latest', loop, redis_conn,
            ),
        )
        print('\n')
        print({

...
def get_uniswapv3_ticks_test():
    ...
    all_start = time.time()
    start = time.time()
    b = w3.eth.call(txn_params, hex(18529042), override_params)
    print(b[0:1000])
    ...
    for i in decoded_bytes_arr[0]:    
        ...
        if (abs(decoded_ticks_arr[-1]['idx']) > 887270):
            print(i.hex())
            print(decoded_ticks_arr[-1])
            print('moose')
        if (decoded_ticks_arr[-1]['liquidity_net'] == 0):
            print(i.hex())
            print(decoded_ticks_arr[-1])
            print('tiger')
    ...
    query = gql(
        """
       query {
        pools(where: {id: "0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640"}, block: {number: 18529042}) {
            id

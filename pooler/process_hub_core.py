...
class ProcessHubCore(Process):
    ...
    def kill_process(self, pid: int):
        _logger = logger.bind(module=f'PowerLoom|ProcessHub|Core:{settings.NAMESPACE}-{settings.INSTANCE_ID}')
    ...
    def run(self) -> None:
        ...
        for callback_worker_file, worker_list in CALLBACK_WORKERS_MAP.items():
            ...
            for each_worker in worker_list:
                # print(each_worker)
                worker_class = getattr(importlib.import_module(f'pooler.callback_modules.{callback_worker_file}'), each_worker['class'])
                worker_count = None

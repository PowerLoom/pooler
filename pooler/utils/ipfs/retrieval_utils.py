...
async def retrieve_block_data(
    ...
    """
    block = await get_dag_block(
        block_dag_cid, project_id, ipfs_read_client=ipfs_read_client
    )
    # handle case of no dag_block or null payload in dag_block
    if not block:
        if (
            data_flag == BlockRetrievalFlags.only_dag_block
            or data_flag == BlockRetrievalFlags.dag_block_and_payload_data
        ):
...
async def retrieve_payload_data(
    ...
    if project_id is not None:
        payload_data = read_text_file(
            settings.ipfs.local_cache_path
            + "/"
            + project_id
            + "/"
            + payload_cid
    ...
    if payload_data is None:
        logger.trace(
            "Failed to read payload with CID %s for project %s from local cache ",
            payload_cid,
            project_id,
        )
        ...
        try:
            ...
            logger.error(
                "Failed to read payload with CID %s for project %s from IPFS : %s",
                payload_cid,
                project_id,
                e,
            )
    ...
    while True:
        ...
        if prev_dag_cid:
            ...
            logger.trace(
                ...
                prev_dag_cid,
                prev_dag_block.height,
                cur_block_cid,
                cur_block.height,
                prev_dag_block.data.payload,
            )
